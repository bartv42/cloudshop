...
class Semaphore {
	 ...
	 */
	public function __construct($lock_name = null, $semaphore_lock_wait = self::DEFAULT_SEMAPHORE_LOCK_WAIT) {
		if(!empty($lock_name)) {
	 ...
	 * @return Semaphore
	 */
	public static function factory($lock_name = null, $semaphore_lock_wait = self::DEFAULT_SEMAPHORE_LOCK_WAIT) {
	...
	public function lock() {
		...
		if(($affected == '0') and !$this->stuck_check()) {
			$this->log(sprintf(__('Semaphore lock "%s" failed (line %s).', AELIA_CS_PLUGIN_TEXTDOMAIN),
												 $this->lock_name,
												 __LINE__));
			return false;
		}
		...
		if($affected != '1') {
			...
			");
			$this->log(sprintf(__('Semaphore "%s" reset to 1.', AELIA_CS_PLUGIN_TEXTDOMAIN),
												 $this->lock_name));
		}
		...
		$this->log(sprintf(__('Set semaphore last lock "%s" time to %s.', AELIA_CS_PLUGIN_TEXTDOMAIN),
											 $this->lock_name,
											 $current_time));
		$this->log(sprintf(__('Set semaphore lock "%s" complete.', AELIA_CS_PLUGIN_TEXTDOMAIN),
											 $this->lock_name));
		return true;
	}
		...
		if(count($filters)) {
			...
			$wpdb->query("
				...
				WHERE
					(option_name = 'aelia_semaphore_" . $this->lock_name . "')
			");
			$this->log(sprintf(__('Incremented the semaphore "%s" by 1.', AELIA_CS_PLUGIN_TEXTDOMAIN),
												 $this->lock_name));
		}
	...
	public function decrement() {
		...
		");
		$this->log(sprintf(__('Decremented the semaphore "%s" by 1.', AELIA_CS_PLUGIN_TEXTDOMAIN),
									 $this->lock_name));
	}
	...
	public function unlock() {
		...
		if($result == '1') {
			$this->log(sprintf(__('Semaphore "%s" unlocked.', AELIA_CS_PLUGIN_TEXTDOMAIN),
									 $this->lock_name));
			return true;
		}

		$this->log(sprintf(__('Semaphore "%s" still locked.', AELIA_CS_PLUGIN_TEXTDOMAIN),
								 $this->lock_name));
		return false;
	}
	...
	private function stuck_check() {
		...
		if($affected == '1') {
			$this->log(sprintf(__('Semaphore "%s" was stuck, set lock time to %s.', AELIA_CS_PLUGIN_TEXTDOMAIN),
												 $this->lock_name,
												 $current_time));
