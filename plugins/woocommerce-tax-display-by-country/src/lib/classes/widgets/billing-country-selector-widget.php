...
class Billing_Country_Selector_Widget extends WP_Widget {
	...
	protected function widget_types() {
		...
		$widget_types = apply_filters('wc_aelia_tdbc_billing_country_selector_widget_types', $widget_types);
		return $widget_types;
	}
	...
	public function __construct() {
		$this->text_domain = WC_Aelia_Tax_Display_By_Country::$text_domain;

		parent::__construct(
			'wc_aelia_tdbc_billing_country_selector_widget',
			'WooCommerce Tax Display by Country - Billing Country Selector',
			array('description' => __('Allow to switch billing country on the fly and ' .
																'display prices with/without tax accordingly.', $this->text_domain),)
		);
	}
	...
	public function widget($widget_args, $instance = array()) {
		...
		if(empty($widget_template_file)) {
			...
			$widget_args['countries'] = apply_filters('wc_aelia_tdbc_billing_country_selector_widget_countries',
